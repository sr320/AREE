...
| Real-study coverage | Three real studies registered. `CALLA2026_OSHV` and `DELISLE2020_OSHV_TEMP` are harmonized at full depth (30,625 and 23,189 evidence records) and pool; `HESSER2024_VCOR` is harmonized but unpoolable. **All three are RNA-seq pathogen challenge, and all three measure `disease_susceptibility` or `disease_resistance` — no registered real study measures a resilience outcome per animal.** Every real top-tier candidate is therefore `disease_associated`, which the evidence cards now state explicitly. The pool needs a second stressor class and a study with a measured survival or viral-load phenotype before AREE can produce `resilience_associated` evidence from real data. |
