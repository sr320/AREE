...
| Random-effects meta-analysis | `src/meta_analysis/` | DerSimonian–Laird, I²/τ², direction-consistency; poolability-aware replication counts; lower-confidence alias de-duplication; fails closed on repeated within-study contrasts |
| Transparent candidate scoring + tiers | `src/prioritize/` | named weighted components; hard gates that a score cannot override; origin/species partitions preserved |
| Evidence cards | `src/reporting/evidence_cards.py` | one per candidate incl. emerging; partition-safe filenames and forest/multi-omics context; limitations and next step |
...
| Test suite | `tests/` (154 tests) | schema, malformed input, duplicate IDs, provenance, mapping confidence, effect sizes, poolability/repeated-contrast guards, origin/species collision isolation, score reproducibility, evidence-card generation, full CLI pipeline, crosswalk construction + real-reference invariants, intake reproducibility, species/assembly validation |
