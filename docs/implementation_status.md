...
| Test suite | `tests/` (130 tests) | schema, malformed input, duplicate IDs, provenance, mapping confidence, effect sizes, meta-analysis, score reproducibility, evidence-card generation, full CLI pipeline, crosswalk construction + selection + real-crosswalk invariants, intake reproducibility + drift detection, species/assembly resolution + validation wiring |
