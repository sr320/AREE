...
| Random-effects meta-analysis | `src/meta_analysis/` | DerSimonian–Laird, I²/τ², direction-consistency; Benjamini–Hochberg `adjusted_p_value` per phenotype/feature-type/origin/species family; tail p-values via the survival function (no underflow to 0); standard errors derived once for the whole table and groups sliced as flat arrays, so a 54k-record, 30k-feature pool runs in about 5 s (was ~95 s); poolability-aware replication counts; lower-confidence alias de-duplication; fails closed on repeated within-study contrasts |
| Transparent candidate scoring + tiers | `src/prioritize/` | named weighted components scored on adjusted p; hard gates that a score cannot override, including adjusted p ≤ 0.05 for the top tier; origin/species partitions preserved; tiers ordered strongest-first |
| Evidence cards | `src/reporting/evidence_cards.py` | every candidate incl. emerging ranked into `candidates.tsv`; a markdown card for each with a significant signal (BH-adjusted p ≤ 0.05 pooled or in a contributing study; `--all-cards` for all); single indexed pass, so a 30k-candidate real pool renders in about a minute; partition-safe filenames, forest/multi-omics context, limitations and next step |
...
| Test suite | `tests/` (176 tests) | schema, malformed input, duplicate IDs, provenance, mapping confidence, effect sizes, poolability/repeated-contrast guards, origin/species collision isolation, score reproducibility, evidence-card generation, full CLI pipeline, crosswalk construction + real-reference invariants, intake reproducibility, species/assembly validation |
| CI | `.github/workflows/ci.yml` | python tests + demo pipeline, real-study path, schema sanity, docs render, all four processed-result Nextflow paths on 26.04, and a stubbed RNA-seq raw-DAG fixture test |
...
|---|---|---|---|
| RNA-seq Nextflow workflow | `workflows/rnaseq/`, `modules/rnaseq/` | **Executed end to end against real public FASTQ** (PRJNA1329250, 11 libraries, 38 processes): FASTQC → fastp → Salmon index → Salmon quant → MultiQC → DESeq2 → standardize → manifest → report. Produced 22,301 genes with real `lfcSE`. Processed mode runs end to end; CI also exercises the raw DAG with tiny paired FASTQ and explicit stubs | real pilot ran natively, **not** inside declared containers; subsampled rather than full depth; only one of six comparisons; the CI stub run validates wiring, not tools or biology |
...
| Meta-analysis on real evidence | `CALLA2026_OSHV` and `DELISLE2020_OSHV_TEMP` pool genome-wide (23,094 shared genes, k = 2). Before family-wise FDR and the significance gate were added on 2026-09-05, 12,658 of those genes qualified as `high_priority_cross_study` — 8,533 with pooled p > 0.05 — because two studies agreeing in sign was the whole test. `HESSER2024_VCOR` still contributes no pooled estimate: its source reports no standard error and no unadjusted p-value, so AREE declines to pool rather than impute. |
| Real-study coverage | Two real studies registered, both RNA-seq and pathogen challenge. `HESSER2024_VCOR` is harmonized; `CALLA2026_OSHV` has a successful subsampled pilot but **no biological result has been harmonized**. Cross-study meta-analysis still needs a full-depth result and an independent poolable study. |
| First raw-reanalysis study | `CALLA2026_OSHV` (PRJNA1329250): design, sample sheet, FASTQ manifest, and run config are complete; one 11-library contrast ran at 1M pairs/library. The full 226 GB run and the other five comparisons remain unexecuted. See [first_raw_reanalysis.md](first_raw_reanalysis.md). |
...
- Full-depth execution of the RNA-seq workflow and real-data execution of the
  other raw paths. RNA-seq now has tiny paired-FASTQ fixtures for a stubbed CI
  DAG test; equivalent methylation/proteomics/metabolomics fixtures remain future work.
